...
// max number of levels for priority based scheduling
#define MAX_QUEUE_LEVEL 4

// the policies for scheduling
...
#define MAX_TLOCAL_KEYS 128
  ...
  // current priority level is stored here
  int currentPriorityLevel;
  
  /* These are the attributes used for user level thread */
  int thread_pid;
...
struct Kernel_Thread *Start_Child_User_Thread(struct User_Context *userContext, bool detached, ulong_t eip);
struct Kernel_Thread *Start_Child_User_Thread_Library(struct User_Context *userContext, bool detached, ulong_t eip);
...
/* Functions for the user library case present here */
