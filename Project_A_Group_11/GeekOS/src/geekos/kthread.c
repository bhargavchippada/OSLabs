...
int currentSchedulingMode = RRS;  // RRS or MLS


// while testing we need some special outputs to get printed
int testMode = 0;    // 0 or 1

int idleThreadMode = 0; // 0 or 1 to exclude the idle thread or not
...
static void Init_Thread(struct Kernel_Thread *kthread, void *stackPage,
    ...
    kthread->totalTime = 0;
    kthread->currentPriorityLevel = 0;
    /*
    ...
    kthread->tot_threads = Malloc(sizeof(int));
    *(kthread->tot_threads) = 1;
    kthread->thread_pid = 1;
    
    kthread->thread_list = Malloc(sizeof(struct Thread_Queue));
    Clear_Thread_Queue(kthread->thread_list);
}


/* To schedule the user level , 
 this functions is like the library which preempts the user thread
*/
void schedule_user_library(struct Kernel_Thread *kthread){
  struct Kernel_Thread *next = 0;
  kthread->thread_ticks++;
  if (kthread->thread_ticks >= MAX_THREAD_TICKS){
    
    /* We can put a sofisticated strategy for scheduling 
       the user threads in this area */
    
    next = Get_Front_Of_Thread_Queue(kthread->thread_list);
    if (next != 0){
      // Puts the current thread to the back of the queue
      Add_To_Back_Of_Thread_Queue(kthread->thread_list, kthread);
      Remove_From_Front_Of_Thread_Queue(kthread->thread_list);
      
      // This is to ensure that the numticks consumed are same for entire thread list
      next->numTicks = kthread->numTicks;
      next->thread_ticks = 0;
      
      if (testMode)
	Print("User Thread Switches to threadid = %d in PID =  %d\n", next->thread_pid, next->pid);
 ...
 * Returns a null pointer if there isn't enough memory.
 */
static struct Kernel_Thread *Create_Thread_Library(int priority, bool detached) {
    struct Kernel_Thread *kthread;
    void *stackPage = 0;
...
/*
 * Set up the a user mode thread, such that the context is not the same as userContext->addr, but is eip, the pointer to the function
 */
/*static*/ void Setup_Child_User_Thread(
                                     struct Kernel_Thread *kthread,
                                     struct User_Context *userContext,
									 uint_t eip) {
    extern int userDebug;
...
/*
 * Start a user-mode thread (i.e., a process), using given user context and the Instruction Pointer
 * Returns pointer to the new thread if successful, null otherwise.
 */
struct Kernel_Thread *Start_Child_User_Thread(struct User_Context *userContext,
					    bool detached,
					    ulong_t eip) {
    struct Kernel_Thread *kthread = Create_Thread(PRIORITY_USER, detached);
    if (kthread != 0) {
        /* Set up the thread, and put it on the run queue */
      Setup_Child_User_Thread(kthread, userContext, eip);
        Make_Runnable_Atomic(kthread);
    }
...
/*
 * Start a user-level thread (i.e., a process with user mode), using given user context and the Instruction Pointer
 * Returns pointer to the new thread if successful, null otherwise.
 * This function does attach the thread to runnable queue
 * This ensures the scheduler knows nothing about this process created till now
 */
struct Kernel_Thread *Start_Child_User_Thread_Library(struct User_Context *userContext,
					    bool detached,
					    ulong_t eip) {
    struct Kernel_Thread *kthread = Create_Thread_Library(PRIORITY_USER, detached);
    if (kthread != 0) {
        /* Set up the thread, and put it on the run queue */
      Setup_Child_User_Thread(kthread, userContext, eip);
    }
...
void Make_Runnable(struct Kernel_Thread *kthread) {
    ...
    if (currentSchedulingMode == MLS){    
      // in case of MLS we put the thread on the priority level 
      // it points to i.e cuurentPriorityLevel

      int prio = kthread->currentPriorityLevel;
      if (kthread->pid == CPUs[cpuID].idleThread->pid){
	prio = MAX_QUEUE_LEVEL - 1;
	CPUs[cpuID].idleThread->currentPriorityLevel = prio;
      }
        
      Enqueue_Thread(&s_runQueue[prio], kthread);
      
    }
    else if (currentSchedulingMode == RRS){
      // in case of RRS we put threads on 0th run queue
...
struct Kernel_Thread *Get_Next_Runnable_Locked(void) {
  ...
  */
  ...
  KASSERT(currentSchedulingMode==RRS || currentSchedulingMode==MLS);
  
  if (currentSchedulingMode == MLS){
    // for the Multi Level Scheduling policy
    int i;
    for (i = 0; i < MAX_QUEUE_LEVEL; i++){
      next = Get_Front_Of_Thread_Queue(&s_runQueue[i]);   // get the head of queue, predefined function
      if (next != 0){
	// if head is valid
    ...
    // for the Round Robin Scheduling Mode, 
    // 0th queue is the only ready queue
    next = Get_Front_Of_Thread_Queue(&s_runQueue[0]);
    if (next != 0){
  ...
  next->numTicks=0;
  if (testMode){
    if (idleThreadMode || (!idleThreadMode && next->pid != 2))
      Print("Dispatching -> %d from level %d\n", next->pid, next->currentPriorityLevel);
  }
...
void Yield(void) {
    ...
    // is the scheduling Policy used is MLS, promotion to be done
    if (currentSchedulingMode == MLS){
      // if current thread if not idle thread 
      if (CURRENT_THREAD->pid != CPUs[cpuID].idleThread->pid){
	// and not in highest priority level, then PROMOTE
	if (CURRENT_THREAD->currentPriorityLevel > 0){
	  CURRENT_THREAD->currentPriorityLevel--;
	}
      }
    }
    ...
    if (testMode){
      if (idleThreadMode || (!idleThreadMode && CURRENT_THREAD->pid != 2))
	Print("Yielding..\n");
    }
...
void Wait(struct Thread_Queue *waitQueue) {
    ...
    // if policy is MLS the promotion to be done
    if (currentSchedulingMode == MLS){
      // if current thread if not idle thread 
      if (current->pid != CPUs[cpuID].idleThread->pid){
	// and not in highest priority level, then PROMOTE
	if (current->currentPriorityLevel > 0){
	  current->currentPriorityLevel--;
	}
      }
    }
