...
static int Sys_PrintEsp(struct Interrupt_State *state) {
  ...
  Print("The address of code at entry point is %lu\n", *((ulong_t*)esp0));
...
/*
 * Create a new user thread with the same context as the current thread and entry point as supplied in arguments
...
static int Sys_SpawnThread(struct Interrupt_State *state) {
    ...
    process = Start_Child_User_Thread(CURRENT_THREAD->userContext, false, state->ebx);
    rc = process->pid;
    Disable_Interrupts();
...
/*To make the current process sleep*/
static int Sys_SleepThread(struct Interrupt_State *state) {
    int time = state->ebx;
    ulong_t endtime = g_numTicks + time;
    while(g_numTicks <= endtime){
      g_numTicks++;
      //	Print("Numticks: %lu\n",g_numTicks);
    }
    return 0;
 ...
 * Create a new user thread with the same context as the current thread and entry point as supplied in arguments
  * Returns: pid of process if successful, error code (< 0) otherwise
  * This process created child thread with same PID as parent
  * The library handles the working of such process
 */
static int Sys_SpawnUserThread(struct Interrupt_State *state) {
   
  Enable_Interrupts();
  int rc;
  struct Kernel_Thread *process = 0;
  
  process = Start_Child_User_Thread_Library(CURRENT_THREAD->userContext, false, state->ebx);
  
  // Appends the created process to thread queue for user level threads
  Add_To_Back_Of_Thread_Queue(CURRENT_THREAD->thread_list, process);
   
  // Update the attributes handling the user level threads
  process->thread_list = CURRENT_THREAD->thread_list;
  process->tot_threads = CURRENT_THREAD->tot_threads;
  *(process->tot_threads) = *(process->tot_threads) + 1;
  process->thread_pid = *(process->tot_threads);
  process->pid = CURRENT_THREAD->pid;                            // making the PID same
...
static int Sys_PS(struct Interrupt_State *state) {
    int maxlen = state->ecx;

    struct Kernel_Thread *kthread;
    int count = 0,ppid,status;
    ...
    while (kthread != 0) {
        ...
        ppid = kthread->owner->pid;
        if(ppid < 0)
            ppid = 0;
        status = 0;
        if(kthread->refCount <=1 && kthread->alive == false)
	  status = 2;
        else if(kthread->alive == false)
	  status = 1;

        Print("%3d\t%4d\t%4d\t%2d\t%s\n", kthread->pid ,  ppid ,  kthread->priority , status,  kthread->threadName);
