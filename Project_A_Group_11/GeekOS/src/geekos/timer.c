...
extern int currentSchedulingMode;      // current scheduling policy
extern int testMode;                   // if testing mode is on
extern int idleThreadMode;
...
unsigned int level_Quantum[MAX_QUEUE_LEVEL] = {4, 5, 6, 7};
...
void Timer_Interrupt_Handler(struct Interrupt_State *state) {
    ...
    //Print("Current %d %d, num %lu\n", current->thread_pid, current->pid, current->numTicks);
    if (current->numTicks >= level_Quantum[current->currentPriorityLevel]) {
        g_needReschedule[id] = true;

	if (testMode){
	  if (idleThreadMode && (!idleThreadMode && current->pid != 2))
	    Print("Pid = %d to be preempted,Now At level %d, ticks consumed=%lu\n", current->pid, current->currentPriorityLevel, current->numTicks);
	}
	...
	if (currentSchedulingMode == MLS){
	  if (current->currentPriorityLevel < MAX_QUEUE_LEVEL - 1){
    ...
    // Calls the user level thread library
    schedule_user_library(current);
