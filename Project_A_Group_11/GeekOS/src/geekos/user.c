...
void Attach_User_Context(struct Kernel_Thread *kthread,
    ...
    strncpy(kthread->threadName, context->name, 20);
...
/*
  to create  a new thread with same context as give 
*/
int SpawnWithContext(struct Kernel_Thread **pThread){
  int rc = 0;
  struct User_Context *userContext = CURRENT_THREAD->userContext;
  struct Kernel_Thread *process = 0;

  /* Start the process! */
  process = Start_User_Thread(userContext, true);
