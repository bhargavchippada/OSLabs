...
DEF_SYSCALL(PrintEsp, SYS_PRINTESP, int, (void),, SYSCALL_REGS_0)

// the syscall for spawn a thread managed by kernel
DEF_SYSCALL(SpawnThread, SYS_SPAWNTHREAD, int, (void *(startFunc)(void)),
	    void *arg0 = startFunc;
	    , SYSCALL_REGS_1)

// the syscall for spawn a user-level thread managed by thread library 
DEF_SYSCALL(SpawnUserThread, SYS_SPAWNUSERTHREAD, int, (void *(startFunc)(void)),
	    void *arg0 = startFunc;
	    , SYSCALL_REGS_1)

// Sycall to handle sleep
DEF_SYSCALL(SleepThread, SYS_SLEEPTHREAD, int, (int time), int arg0 = time; , SYSCALL_REGS_1)
